...
export default function MonthlySummary({ user }) {
  ...
  const done = TACHES_MENSUELLES.filter(t => isMensuelChecked(user, t.id)).length
    ...
    <div className="encart encart-monthly">
      ...
      <div className="monthly-tasks">
        {TACHES_MENSUELLES.map(tache => {
          const checked = isMensuelChecked(user, tache.id)
          return (
            <label key={tache.id} className={`checkbox-row ${checked ? 'checked' : ''}`}>
              <input
                type="checkbox"
                checked={checked}
                onChange={e => toggleMensuel(user, tache.id, e.target.checked)}
              />
